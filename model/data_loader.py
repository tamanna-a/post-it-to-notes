...
class data_loader:
    def __init__(self, path, batch_size):
        ...
        self.samples = [] #list of Sample object, which contain text & file name
        self.currIdx = 0
        self.charList = []
        ...
        for line in f:
            if not line or line[0] == '#':
                continue
            lineSplit = line.strip().split(' ') #splits into [a01-000u-00, ok, 154,..]
            assert len(lineSplit) >= 9
            fileNameSplit = lineSplit[0].split('-') #splits into [a01, 000u,00]
            fileName = path + 'lines/' + fileNameSplit[0] + '/' + \
                       fileNameSplit[0] + '-' + fileNameSplit[1] + '/' + lineSplit[0] + '.png'

            # lineSplit[8] = A|MOVE|to|stop|Mr.|Gaitskell|from
            gtText = lineSplit[8].strip(" ").replace("|", " ")

            chars = chars.union(set(list(gtText)))
            self.samples.append(Sample(gtText, fileName)) #contains text and filename
        ...
        #split file contain in form: c04-110
